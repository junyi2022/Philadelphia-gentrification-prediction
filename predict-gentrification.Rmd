...
model <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
...
We want a better sensitivity. A good sensitivity means that our model could correctly identify  True Positive, the gentrification block groups. Effective sensitivity empowers governments to implement targeted interventions in these areas, mitigating or reducing future gentrification.

We employ a threshold of 0.45 to optimize the balance between cost and benefit. This threshold selection aims to gain a balance where the costs associated with false positives and false negatives are minimized while maximizing the benefits of correctly identifying gentrification cases.[Appendix]
...
testProbs <- 
  testProbs %>%
  mutate(predOutcome  = as.factor(ifelse(testProbs$Probs > 0.45 , 1, 0)))
...
cvFit <- train(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park+station.factor+Burglary+holc_grade,
  data = data2018 %>%
    dplyr::select(-GEOID, -geometry),
  method = "glm",
...
testProbs <- data.frame(Outcome = as.factor(Gentrification.test$Gentrification),
                        Probs = predict(model6, Gentrification.test, type= "response"))
...
testProbs <- 
  ...
  mutate(predClass_50 = as.factor(ifelse(testProbs$Probs > 0.5 , 1, 0)),
         predClass_45 = as.factor(ifelse(testProbs$Probs > 0.45 , 1, 0)),
...
testProbs %>%
  ...
  group_by(Variable) %>%
  summarize(Sensitivity = round(yardstick::sens_vec(Outcome,factor(Value)),2),
            Specificity = round(yardstick::spec_vec(Outcome,factor(Value)),2),
...
validationProbs <- 
  validationProbs %>%
  mutate(predOutcome  = as.factor(ifelse(validationProbs$Probs > 0.45 , 1, 0)))
...
caret::confusionMatrix(validationProbs$predOutcome, validationProbs$Outcome, 
                       positive = "1")
...
model1 <- glm(Gentrification ~ MedHHIncome+MedNumRooms+pctWhite+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation,
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model2 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model3 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model4 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park+station, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model5 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park+station+Burglary+holc_grade, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model6 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park+station.factor+Burglary+holc_grade, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model7 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school.factor+park+station.factor+Burglary+holc_grade, 
             data = as.data.frame(Gentrification.training) %>% 
                     dplyr::select(-GEOID, -geometry),
                  family="binomial" (link="logit"))

model8 <- glm(Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
...
#model5
cvFit <- train(
  Gentrification ~ MedHHIncome+pctBlack+pctChild+pctYouth+pctFamilyHH+pctOccupied+pctRent+pctHSEducation+
                school+park+station+Burglary+holc_grade,
  data = data2018 %>%
    dplyr::select(-GEOID, -geometry),
  method = "glm",
