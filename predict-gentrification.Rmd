...
Education amenities serve as an important independent variable in our analysis. When a block group offers quality educational resources, it often attracts individuals seeking access to such amenities. This influx of residents seeking quality education can contribute to an increase in housing values and may consequently lead to gentrification within the area. To quantify this effect, we calculate the total count of schools within each group as a variable of interest. The school data comes from The School District of Philadelphia's [School Lists](https://www.philasd.org/performance/programsservices/open-data/school-information/#school_lists) dataset.  
...
Proximity to parks reflects the physical environment of a community. Individuals often prefer to reside near green spaces and parks for a more desirable living environment. When a block group offers such favorable conditions, it tends to attract new residents, potentially leading to gentrification. To quantify this impact, we calculate the count of parks located within 500 feet of each unit as a variable of interest in our analysis. The park data comes from OpenDataPhilly [Parks & Recreation Program Sites](https://opendataphilly.org/datasets/parks-recreation-program-sites) dataset.
...
Residents are drawn to areas with enhanced safety measures. To assess the safety level of each block group, we utilize burglary data as a metric.The crime data comes from Opendataphilly [Crime Incidents](https://opendataphilly.org/datasets/crime-incidents/) dataset.
...
Moreover, we want to explore the  the influence of historic redlining on current gentrification trends. For example, whether gentrification tends to occur more frequently in areas that historically received better grades. 
...
The plot shows an initial exploration of the relationship between our continuous independent variables and gentrification, our dependent variable. 
...
Below is the table showcasing our final model. For specific details on model selection criteria, please refer to the appendix. Our analysis indicates that the median number of rooms, the percentage of individuals with Bachelor's degrees or higher, and population density are statistically significant predictors in the model.
...
A sensitivity of 0.25 indicates that the model correctly identifies only 25% of gentrified areas, which is relatively low. Combined with a low positive predictive value, this suggests that the model is relatively ineffective at accurately predicting gentrified areas. However, a specificity of 0.82 suggests that it is relatively good at identifying areas that are not gentrified.


### 2017 Cross Validation

cross validate using 2017 data
...
Here, the ROC value is 0.71, which is slightly lower compared to the model that used 2018 data for cross-validation, which had an ROC of 0.74. Despite this slight decrease, the model still demonstrates a relatively good ability to differentiate between positive and negative outcomes.
...
The blue plot shows a high density of predicted probabilities close to 0, which indicates that the model confidently predicts a large number of areas as not gentrified. This is consistent with the relatively high specificity observed (0.82), as the model successfully identifies a significant proportion of true negatives. The red plot, however, shows most predicted probabilities clustered around the lower end (below 0.2), with a very small portion exceeding this range. This distribution explains the low sensitivity (0.25) because the model assigns low probabilities to most actual gentrified areas, failing to recognize them as such.
...
To summarize, this model employs demographic indicators such as the percentage of minority groups, median household income, and changes in household income to analyze gentrification in Philadelphia. It incorporates additional socio-economic and urban amenities data to predict areas undergoing gentrification. The model focuses on accurately identifying true positive areas, aiming to pinpoint actual gentrified locations effectively. It has been tested with data from the previous year and successfully identified all the gentrified areas noted.
...
Additionally, by tracking the effects of gentrification, the model helps mitigate adverse outcomes such as the displacement of lower-income families and the erosion of cultural heritage. It also provides community advocates and local stakeholders with data-driven insights, supporting efforts to advocate for inclusive urban policies.
...
We chose Model 6 as the final model because it performed well on several evaluation metrics. When comparing McFadden R-squared, AIC, and AUC together, Model 6 showed better predictive power and model fit. In addition, Model 6's AUC demonstrated greater stability during cross-validation, further solidifying its position as the model of choice for the analysis.
